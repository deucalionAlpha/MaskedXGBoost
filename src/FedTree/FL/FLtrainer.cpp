...
//            parties[i].dh = server.dh;
    ...
    int n_bins = model_param.max_num_bin; //default 32
...
//                        global_hist_fid_data[i] = batch_idxs[pid][hist_fid_data[i]];
                    ...
                    LOG(INFO) << "Vertical Compute His time = " << training_time.count() << "s";
                ...
                auto his_stop = timer.now();
                std::chrono::duration<float> his_time = his_stop - his_start;
                LOG(INFO) << "Whole Vertical Compute His time = " << his_time.count() << "s";
...
#pragma omp parallel for
        ...
        for (int t = 0; t < params.gbdt_param.tree_per_rounds; t++) {
            ...
            for (int l = 0; l < params.gbdt_param.depth; l++) {
                ...
                for (int pid = 0; pid < parties.size(); pid++)
                {
                    parties_n_columns[pid] = parties[pid].dataset.n_features(); // origin code
                    int n_bins = parties[pid].booster.fbuilder->cut.cut_points_val.size();
                    all_bins += n_bins; //real bins
                    if(pid == 0) begin_party[pid] = 0;
                ...
                auto start = timer.now();
                #pragma omp parallel for
                for (int pid = 0; pid < parties.size(); pid++) { //both AP and PP
                    int n_bins = parties[pid].booster.fbuilder->cut.cut_points_val.size();
                    parties_n_bins[pid] = n_bins;
                    // LOG(INFO) << "pid :" << pid << " " << n_bins;
                    int n_max_splits = n_max_nodes * n_bins;
                    int n_column = parties[pid].dataset.n_features();
                    int n_partition = n_column * n_nodes_in_level;
                    auto cut_fid_data = parties[pid].booster.fbuilder->cut.cut_fid.host_data();
...
//                        global_hist_fid_data[i] = batch_idxs[pid][hist_fid_data[i]];
                    ...
                    parties[pid].booster.fbuilder->compute_histogram_in_a_level(l, n_max_splits, n_bins,n_nodes_in_level, hist_fid_data, missing_gh, hist);
                ...
                for (int pid = 0; pid < parties.size(); pid++) { //both AP and PP
                //MaskedSMM
                    parties[pid].booster.fbuilder->variance = params.variance;
                    parties[pid].booster.fbuilder->masked_computegain_smm(gain_smm, l, begin_party[pid], all_bins, missing_gh, n_bins_new, global_hist_fid.host_data());
                }
                ...
                server.booster.fbuilder->get_best_gain_in_a_level(gain_smm, best_idx_gain, n_nodes_in_level, n_bins_new); 
