...
#pragma omp parallel for
    for (int fid = 0; fid < n_column; fid++) {
        for (int i = csc_col_ptr_data[fid]; i < csc_col_ptr_data[fid + 1]; i++) {
            ...
            //LOG(INFO) << "bid of " << row * n_column + fid  << " " << int(bid);
        }
    }
...
void HistTreeBuilder::masked_computegain_smm(SyncArray<float_type> &gain, int level, int &index, int all_bins, SyncArray<GHPair> &missing_gh, int n_bins, int *hist_fid){
    auto compute_gain = []__host__(double sum_g, double sum_h, double leftg, double lefth, double rightg, double righth, float_type min_child_weight, float_type lambda) -> double {
            if (lefth >= min_child_weight && righth >= min_child_weight)
                return (leftg * leftg) / (lefth + lambda) + (rightg * rightg) / (righth + lambda) -
    ...
    std::chrono::high_resolution_clock timer;
    auto smm_start = timer.now(),  stop = timer.now();
    std::chrono::duration<float> training_time;
    ...
    auto cut_col_ptr_data = cut.cut_col_ptr.host_data();
    int all_splits = cut_col_ptr_data[n_column];
    // LOG(INFO) << "all_splits " << cut_col_ptr_data[n_column];
    
    if(n_nodes_in_level == 1)
    {
        auto &dense_bin_id = this->dense_bin_id;
        auto n_instances = this->n_instances;
        //this->AllsplitMatrix.resize(n_instances, all_splits);
        MatrixXd splitMatrix(n_instances, all_splits);
        MatrixXd matrix_maskedgrad(all_splits, n_instances); 
        MatrixXd matrix_maskedhess(all_splits, n_instances);
        MatrixXd ghpair(2,n_instances),leftgh(2,all_splits), rightgh(2,all_splits);
        stop = timer.now();
        ...
        for(int i = 0; i < all_splits; i++){
            ...
            */
            ...
            if (nodes_data[nid].is_valid){
                ...
                if (default_to_left_gain > default_to_right_gain)
                    gain_data[i+index] = default_to_left_gain;
                else
                    gain_data[i+index] = - default_to_right_gain;
            }
            else{
                gain_data[i+index] = 0;
            }
        ...
        for (int i = 0; i < n_nodes_in_level; ++i) { // 遍历每个节点的instance build split matrix 这里可能通过n_nodes_in_level/2少一部分计算。
            ...
            for(int j = 0; j < all_splits; j++){
                ...
                if (nodes_data[nid].is_valid){
                    ...
                    leftgh(1,j) += p_missing_gh.h;

                    float_type default_to_right_gain = std::max((double)0, compute_gain(sum_g, sum_h, leftgh(0,j), leftgh(1,j), sum_g-leftgh(0,j), sum_h-leftgh(1,j), mcw, lambda));
...
void HistTreeBuilder::compute_histogram_in_a_level(int level, int n_max_splits, int n_bins, int n_nodes_in_level,
...

    {
        ...
        if (n_nodes_in_level == 1) {
            ...
            for(int fid = 0; fid < n_column; fid++){
                for(int iid = 0; iid < n_instances; iid++) {
                    unsigned char bid = dense_bin_id_data[iid * n_column + fid];  
                    //dense_bin_id_data[userid] = binid;
                    //LOG(INFO) << "Bin id of " << iid << " " << fid <<  " is " << bid;
                    if (bid != max_num_bin) {
            ...
            for (int i = 0; i < n_nodes_in_level / 2; ++i) {
                ...
                //compute histogram
                {
                    ...
                    auto idx_begin = node_ptr.host_data()[nid0];
                    auto idx_end = node_ptr.host_data()[nid0 + 1];
                    // LOG(INFO) << "In hist idx_begin  " << idx_begin << " idx_end " <<  idx_end;
    ...
    // LOG(INFO) << "Real Vertical time = " << training_time.count() << "s";
    return;
}
...
//            rch_gh = rch_gh + p_missing_gh;
            ...
            if (default_to_left_gain > default_to_right_gain)
                ...
                gain_data[i] = -default_to_right_gain; //negative means default split to right
            // if(i==4){
            //     //LOG(INFO) << pid << " " << n_split << " " << n_bins << " " << n_column;
            //     LOG(INFO) << "Check Origin gain[0]" << default_to_left_gain << " "  << default_to_right_gain;
            // }
        } 
        else{
            gain_data[i] = 0;
